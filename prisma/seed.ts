...
async function main() {
  ...
  const createdPkCards = await Promise.all(
    pokemonCards.map((c) =>
      prisma.card.upsert({
        where: { id: undefined as unknown as string }, // force create
        update: {},
        create: { ...c, series: "Pokemon", seriesId: pokemon.id },
      }).catch(() =>
        // upsert won't work without unique field match, use create directly
        prisma.card.create({ data: { ...c, series: "Pokemon", seriesId: pokemon.id } })
      )
    )
  );

  const createdOpCards = await Promise.all(
    onepieceCards.map((c) =>
      prisma.card.create({ data: { ...c, series: "One Piece", seriesId: onepiece.id } })
    )
  );
  ...
  // ==========================================
  const rates = [
    { currency: "USD", rateToAed: 3.6725, source: "manual", validAt: new Date() },
    { currency: "EUR", rateToAed: 3.98, source: "manual", validAt: new Date() },
    { currency: "JPY", rateToAed: 0.0245, source: "manual", validAt: new Date() },
    { currency: "GBP", rateToAed: 4.64, source: "manual", validAt: new Date() },
  ];
